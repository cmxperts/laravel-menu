                    ...
                    <div class="form-group">
                        ...
                        @if(request()->has('action'))
                            <button type="button" onclick="createNewMenu()" name="save_menu"
                                    class="btn btn-primary menu-save ml-2">Create Menu
                            </button>
                        @elseif(request()->has('menu'))
                            <button type="button" onclick="actualizarMenu(false)" name="save_menu"
                                    class="btn btn-primary menu-save ml-2">Save Menu
                            </button>
                        @else
                            <button type="button" onclick="createNewMenu()" name="save_menu"
                                    class="btn btn-primary menu-save ml-2">Create Menu
                    ...
                    <div class="jumbotron jumbotron-fluid p-2">
                        <div class="container">
                            ...
                            <div class="alert alert-info">
                                <p class="">Place each item in the order you prefer. Click <i class="la la-angle-down"
                                                                                              aria-hidden="true"></i> to
                                    the right of the item to display more configuration options.</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="dd nestable-menu" id="nestable">
                            <ol class="dd-list">
                                @foreach($menus as $key => $m)
                                    @include('cmxperts::menu.partials.loop-item', ['key' => $key])
                                @endforeach
                            </ol>
                        </div>
                    @endif
