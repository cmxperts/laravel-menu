<li data-id="{{$m['id']}}" class="dd-item mb-2">
    <div class="card-header">
        <span class="dd-handle"><i class="la la-arrows" aria-hidden="true"></i></span>
        ...
        <div class="card-link float-right" data-toggle="collapse" href="#collapse{{$m['id']}}">
            <span class="item-controls">
                <span class="item-type">
                    @if($m['type'] == 'menu')
                        Link
                    @elseif($m['type'] == 'breaker')
                        Separator
                    @endif
                    <i class="la la-angle-down narrow-icon" aria-hidden="true"></i></span>
            </span>
        </div>
    </div>
    {{-- @if($key == 0) show @endif --}}
    ...
    @if (isset($m['child']) && count($m['child']) > 0)
        <ol class="dd-list">
            @foreach($m['child'] as $_m)
                @include('cmxperts::menu.partials.loop-item', ['m' => $_m, 'key' => 1])
            @endforeach
        </ol>
    @endif
</li>
